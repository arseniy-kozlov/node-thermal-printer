...
module.exports = {
  ...
  init: function(initConfig){
    ...
    } else if (initConfig.type === printerTypes.CUSTOM) {
      config = require('../configs/customConfig');
    } else {
  ...
  printQR: function(str, settings){
    ...
    } else if (printerConfig.type === printerTypes.CUSTOM) {
      append(custom.printQR(str, settings));
    } else {
  ...
  printBarcode: function(data, type, settings){
    if(printerConfig.type === printerTypes.EPSON) {
      ...
      append(custom.printBarcode(data, type, settings));
    } else {
  ...
  maxiCode: function(data, settings){
    if(printerConfig.type === printerTypes.EPSON) {
      ...
      append(custom.maxiCode(data, settings));
    } else {
  ...
  code128: function(data, settings) {
    if (printerConfig.type === printerTypes.STAR) {
      ...
      append(custom.code128(data, settings));
    } else {
  ...
  pdf417: function(data, settings) {
    if (printerConfig.type === printerTypes.STAR) {
      ...
      append(custom.pdf417(data, settings));
    } else {
    ...
    try {
      ...
      if(image.slice(-4) === ".png"){
        if (printerConfig.type === printerTypes.STAR){
          ...
          custom.printImageEpson(image, function(response){
            if(response) append(response);
            callback(response);
          });
        } else {
  ...
  printImageBuffer: function(buffer, callback){
    ...
    png.on('parsed', function() {
      if (printerConfig.type === printerTypes.STAR){
        ...
        custom._printImageBufferEpson(this.width, this.height, this.data, function(response){
          if(response) append(response);
          callback(response);
        });
      } else {
